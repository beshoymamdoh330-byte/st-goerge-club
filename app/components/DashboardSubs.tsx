...
import React, { useState, useEffect } from 'react'
import Link from 'next/link'
import { CardChecklist, ArrowLeftShort } from 'react-bootstrap-icons'
import { useThemeContext } from '../assets/contexts'
...
export default function DashboardSubs() {
        ...
        const getSubsCount = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    else if (results?.data) count = results.data.length
                    else if (results?.$values) count = results.$values.length
                ...
                console.error("Error fetching subs count:", err)
    ...
    return (
        <Link href="/dashboard/subs" className="block w-full group mb-4">
            <div className={`p-6 rounded-3xl border transition-all duration-300 hover:-translate-y-1 cursor-pointer flex items-center justify-between ${
                isDark
                    ? 'bg-slate-900/80 border-slate-800 text-slate-100 shadow-lg hover:border-blue-600/50 hover:shadow-blue-600/10'
                    : 'bg-white/90 border-slate-200 text-slate-800 shadow-md hover:border-blue-500 hover:shadow-blue-500/10'
            }`}>
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-bold group-hover:text-blue-600 transition-colors capitalize">All Subs</h3>
                        <p className="text-xs text-slate-500 dark:text-slate-400">Click to view and manage all subscription plans</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <ArrowLeftShort size={28} className="text-slate-400 group-hover:text-blue-600 group-hover:-translate-x-1 transition-all" />
                </div>
            </div>
        </Link>
    )
